...
public class ClientDBManager {
	...
	public ClientDBManager() 
	{
		try 
		{
			...
			{
				stmt.executeUpdate("CREATE TABLE client(id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,name varchar(20))");
			}

			addClientStmt = conn.prepareStatement("INSERT INTO person (firstName, surname)  VALUES (?, ?)");

			getClientStmt = conn.prepareStatement("SELECT * FROM client");
			
			deleteAllClientStmt = conn.prepareStatement("DELETE FROM client");
			
			findClientStmt = conn.prepareStatement("SELECT id FROM client WHERE firstName, surname = ?");
